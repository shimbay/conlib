...
template <typename T> BlockingQueue<T>::BlockingQueue(size_t max_size) {
  assert(max_size > 0);
...
template <typename T> ErrorCode BlockingQueue<T>::offer(T &&t, std::chrono::milliseconds timeout) {
  if (timeout.count() == 0) {
...
template <typename T> ErrorCode BlockingQueue<T>::take(T &res, std::chrono::milliseconds timeout) {
  if (timeout.count() == 0) {
...
template <typename T> size_t BlockingQueue<T>::len() { return m_queue.size(); }
