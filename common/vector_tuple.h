...
struct VectorTuple<T, Ts...>: VectorTuple<Ts...> {
...
template<class T, class... Ts>
struct ElemTypeHolder<0, VectorTuple<T, Ts...>> {
    typedef std::vector<T> type;
};

template<size_t k, class T, class... Ts>
struct ElemTypeHolder<k, VectorTuple<T, Ts...>> {
    typedef typename ElemTypeHolder<k - 1, VectorTuple<Ts...>>::type type;
};

template<class T>
void emplace(VectorTuple<T> &v, T &&t) {
    v.tail.emplace_back(std::forward<T>(t));
}

template<class T, class... Ts>
void emplace(VectorTuple<T, Ts...> &v, T &&t, Ts &&... ts) {
    v.tail.emplace_back(std::forward<T>(t));
