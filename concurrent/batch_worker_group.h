...
template <typename... Ts> class BatchWorkerGroup {
public:
  BatchWorkerGroup(size_t worker_num, size_t max_batch_size,
...
BatchWorkerGroup<Ts...>::BatchWorkerGroup(size_t worker_num, size_t max_batch_size,
                                          std::chrono::milliseconds batch_timeout)
    : m_worker_num(worker_num), m_max_batch_size(max_batch_size), m_batch_timeout(batch_timeout) {
...
template <typename... Ts> BatchWorkerGroup<Ts...>::~BatchWorkerGroup() {
...
template <typename... Ts> ErrorCode BatchWorkerGroup<Ts...>::submit(Ts &&... ts) {
  std::unique_lock<std::mutex> lock(m_mutex);
  if (m_closed) {
    return QueueClosed;
  }
...
template <typename... Ts> size_t BatchWorkerGroup<Ts...>::size() { return m_data_size; }

template <typename... Ts> void BatchWorkerGroup<Ts...>::_work() {
  while (!m_closed) {
    VectorTuple<Ts...> data;
